...
def createStartScreen(data):
		data.startScreenButtons.append(Button(data.width-data.margin-10-200,0+data.margin,200,80,"darkgrey",5,"grey","Classic"t))
		...
		data.startScreenButtons.append(Button(data.margin+50,data.height/4*3,200,80,"darkgrey",5,"grey","Guide"))
...
def drawGuide(canvas,data):
		if data.gameState=="Guide":
				text="""
				Player1: 
				Use Arrow Keys To Move, "n" To Attack, "m" To ActivateSkill

				Player2: 
				Use "wasd" To Move, "1" To Attack, "2" To ActivateSkill

				At Anywhere In The Game, Press "Esc" To Head Back To The Start Screen

				"""
				canvas.create_text(data.width/2-120,data.height/2,text=text,font="Arial 12 bold",fill="white")
